...
from queries import (
    ...
    picks_bans_df, match_stats_df, players_query
)
...
champions_tab, matchup_tab = st.tabs(["Champion Pool", "Matchups"])
...
roles = st.sidebar.selectbox('Role', ['baron', 'jungle', 'mid', 'dragon', 'sup'])
player = st.sidebar.selectbox('Player', sum(run_query(players_query(
    patches, phases, teams, [roles], tournaments
)), ()))

with champions_tab:
    player_df = pd.DataFrame(
        run_query(general_stats_query(patches, phases, teams, [roles], tournaments, columns='pick', player=player)),
        columns=[
            'Champion', 'AVG Kills', 'AVG Deaths', 'AVG Assists', 'AVG KDA',
...
with matchup_tab:
    ...
    champion = st.selectbox('Champion', pd.DataFrame(
        run_query(general_stats_query(patches, phases, teams, [roles], tournaments, columns='pick', player=player)),
        columns=[
            'Champion', 'AVG Kills', 'AVG Deaths', 'AVG Assists', 'AVG KDA',
            'AVG DDPM', 'AVG DTPM', 'AVG GPM', 'AVG DDPG', 'AVG DTPG',
    ...
    maps = sum(run_query(champion_maps_query(
        patches, phases, teams, selected_roles, tournaments, champion, player)), ())
    q_with = pd.DataFrame()
